...
bool isComment(const string &line) {
  for (int i = 0; i < line.size(); i++) {
    if (line[i] == space) {
      continue;
    } else if (line[i] == slash && line[i + 1] == slash) {
      return true;
    }
    return false;
  }
...
void searchCommentsInFile(const string name, vector<int> &firstOccurence, vector<int> &amountOfOccurence,
...
void writeCommentsInFile(const string name, const vector<string> &comments, const vector<int> &firstOccurence,
                         const vector<int> &amountOfOccurence) {
  ...
  for (int i = 0; i < comments.size(); i++) {
    output << firstOccurence[i] << limiter << amountOfOccurence[i] << limiter << comments[i] << "\n";
  }
  output.close();
}
...
int main() {
  ...
  while (state != 0) {
    switch (state) {
      case 1:cout << "Print file Name\n";
        cin >> fileName;
        printFileInConsole(fileName);
        cin >> state;
        break;
      case 2:cout << "Print file Name, where you will search comments\n";
        cin >> fileName;
        searchCommentsInFile(fileName, firstOccurence, amountOfOccurence, comments);
        cin >> state;
        break;
      case 3:cout << "Print file Name, where you will write comments\n";
        cin >> fileName;
